...
public class WarcryMenu extends GameInventory {
    ...
    public WarcryMenu(Player player) {
        ...
        ItemBuilder rareLockedGlass = new ItemBuilder(Material.STAINED_GLASS_PANE).durability(11).name(ChatColor.BLUE + "Rare Warcry").lore(ChatColor.GRAY + "Random unlock from a Cosmeticrate");
        for (int i = 9; i < 18; i++) {
            addClickableItem(i, rareLockedGlass);
        }

        ItemBuilder epicLockedGlass = new ItemBuilder(Material.STAINED_GLASS_PANE).durability(1).name(new ColorBuilder(ChatColor.GOLD).bold().create() + "Epic Warcry").lore(ChatColor.GRAY + "Random unlock from a Cosmeticrate");
        for (int i = 0; i < 5; i++) {
            addClickableItem(i, epicLockedGlass);
        }

        ItemBuilder legendaryLockedGlass = new ItemBuilder(Material.STAINED_GLASS_PANE).durability(2).name(new ColorBuilder(ChatColor.LIGHT_PURPLE).bold().create() + "Legendary Warcry").lore(ChatColor.GRAY + "Random unlock from a Cosmeticrate");
        for (int i = 5; i < 9; i++) {
        ...
        addClickableItem(26, Cosmetic.Item.WARCRY_NONE.getItem());
